...
import { ShoppingBag, MapPin, Hash, ExternalLink } from "lucide-react";
    ...
    <AppLayout>
      ...
      {products.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {products.map((p) => (
            <article key={p.id} className="neon-card text-neon-pink p-4 hover:border-glow-pink transition-shadow">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] px-2 py-0.5 rounded-full border border-neon-green/50 text-neon-green">
                  {p.stock} left
                </span>
              </div>
              ...
              <div className="mt-4 flex items-end justify-between">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-neon-pink/15 border border-neon-pink text-neon-pink border-glow-pink text-sm font-medium"
                >
